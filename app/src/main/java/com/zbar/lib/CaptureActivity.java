...
public class CaptureActivity extends BaseActivity implements Callback {
	...
	private ButtonFlat btnOpenLight;
	private ButtonFlat btnChooseImg;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		Config.setTranslucentStatus(true, this);
		// 初始化 CameraManager
		...
		//按钮
		btnOpenLight = (ButtonFlat) findViewById(R.id.btnOpenLight);
		...
		switch (v.getId()) {
			case R.id.btnOpenLight:
				if (isOpenLight == false) {
					btnOpenLight.setText(getString(R.string.scan_btn_close_light));
					light();
				} else {
	...
	public void handleDecode(String result) {
		...
		intent = new Intent(CaptureActivity.this, c);
				...
				case Config.REQ_OPEN_IMG:
					...
					new Thread(new Runnable() {
						...
						public void run() {
							...
							if (result == null) {
								Looper.prepare();
								Toast.makeText(getApplicationContext(), R.string.fail_to_scan, Toast.LENGTH_SHORT)
										.show();
								Looper.loop();
							} else {
								// 数据返回
